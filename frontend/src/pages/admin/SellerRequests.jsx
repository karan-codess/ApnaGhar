          ...
          {requests.length === 0 ? (
            ...
            <div className="grid grid-cols-[repeat(auto-fill,minmax(350px,1fr))] gap-6 max-sm:grid-cols-1">
              {requests.map((request) => (
                <div
                  key={request._id}
                  className="border border-[#f1f5f9] rounded-2xl p-6 bg-[#f8fafc] transition-all duration-300 ease-in-out hover:-translate-y-1"
                >
                  <div className="flex items-center gap-4 mb-5">
                    ...
                    <div>
                      <div className="font-bold text-[1.1rem] text-text-main">
                        {request.name}
                      </div>
