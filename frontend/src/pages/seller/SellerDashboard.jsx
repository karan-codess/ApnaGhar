    ...
    <>
      <header className="flex flex-col md:flex-row justify-between items-start mb-8 flex-wrap gap-6 md:gap-6">
        <div className="min-w-0 w-full md:w-auto md:min-w-[280px]">
          <h1 className="text-[1.5rem] sm:text-[1.75rem] font-extrabold text-text-main mb-1">
            Seller Dashboard
          </h1>
        ...
        <div className="flex gap-3 flex-wrap w-full md:w-auto">
          ...
          <Link
            to="/add-property"
            className="btn btn-primary flex items-center gap-2 font-bold py-3 px-5 flex-1 justify-center whitespace-nowrap"
          >
            <HiPlus size={20} />
            Add New
          </Link>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-[repeat(auto-fit,minmax(240px,1fr))] gap-4 sm:gap-5 mb-12">
        {statCards.map((card, i) => (
          <div
            key={i}
            className="bg-white p-6 rounded-[1.25rem] border border-[#f1f5f9] shadow-[0_4px_20px_rgba(0,0,0,0.02)]"
            style={{ "--card-color": card.color }}
          >
            <div className="w-10 h-10 rounded-xl bg-[#f1f5f9] flex items-center justify-center mb-5">
              <card.icon size={20} />
            </div>
            <div className="text-[#64748b] text-[0.8125rem] font-semibold mb-1">
              {card.title}
            </div>
            <div className="text-[1.5rem] font-extrabold text-text-main">
              {card.value}
            </div>
          </div>
        ))}
          ...
          <>
                  ...
                  renderActions={() => (
                    <div className="flex-1 flex gap-[0.4rem]">
                      <button
                        ...
                        className={`btn btn-outline flex-1 p-2 text-[0.75rem] flex items-center justify-center gap-1 ${
                          p.status === "sold"
                      ...
                      <button
                        ...
                        className="btn btn-outline flex-1 p-2 text-[0.75rem] text-[#ef4444] border-[#fee2e2] flex items-center justify-center gap-1 hover:bg-red-50"
                      >
            ...
            {filteredProperties.length > 3 && (
              <div className="text-center mt-10">
                <Link
                  to="/my-properties"
                  className="btn btn-outline py-3 px-10 font-extrabold text-[0.9rem] rounded-xl bg-white shadow-[0_4px_12px_rgba(0,0,0,0.05)] inline-flex items-center gap-2"
                >
                  Show More Listings
                  <HiOutlinePencilAlt
                    size={18}
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-8 mb-8">
        <div className="bg-white rounded-3xl border border-[#f1f5f9] p-6 shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
          <h2 className="text-[1.125rem] font-extrabold text-text-main mb-1">
            Recent lead inquiries
          </h2>
          <p className="text-[#64748b] text-[0.8125rem] mb-6">
            new messages from potential buyer
          </p>
          <div className="flex flex-col gap-5">
            {inquiries.map((inq, i) => (
              ...
              >
                ...
                <div className="text-right flex-1">
                  ...
                  <span
                    className={`py-0.5 px-2 rounded-full text-[0.65rem] font-extrabold uppercase ${
                      inq.status === "read"
            ...
            {inquiries.length === 0 && (
              <p className="text-center text-[#64748b] p-4">
                No recent inquiries
              </p>
            )}
          </div>
        ...
        <div className="bg-white rounded-3xl border border-[#f1f5f9] p-6 shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="p-4 rounded-2xl bg-[#f9fafb]">
              <h4 className="text-[#4b5563] mb-1 font-bold text-[0.875rem]">
                Market insight
              </h4>
