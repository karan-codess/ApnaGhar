...
const PropertyCard = ({
  ...
  const statusBadgeClass = (status) =>
    `text-white px-3 py-1 rounded-full text-xs font-extrabold uppercase backdrop-blur shadow-[0_2px_8px_rgba(0,0,0,0.1)] ${
      status === "sold" ? "bg-[#64748b]" : "bg-[#10b981]"
    }`;

  const wishlistButtonClass = (isWishlisted) =>
    `flex items-center justify-center w-8 h-8 rounded-full border-none cursor-pointer shadow-[0_2px_8px_rgba(0,0,0,0.1)] transition-all duration-200 z-[15] hover:text-red-500 hover:scale-110 hover:bg-white ${
      isWishlisted ? "text-red-500 bg-white" : "text-[#64748b] bg-white/90"
    }`;
  ...
  return (
    <div className="fade-in group flex flex-col bg-white rounded-[1.25rem] overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.05)] transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] border border-[#e2e8f0] relative w-full hover:-translate-y-2 hover:shadow-[0_12px_30px_rgba(0,0,0,0.1)] max-sm:max-w-[420px] max-sm:mx-auto">
      <Link
        ...
        className="no-underline text-inherit flex flex-col w-full"
      >
          ...
          <div className="absolute top-4 left-4 right-4 flex justify-between items-center z-10">
            <div className="flex gap-2">
              {renderActions ? (
                ...
                <span className="bg-white/90 px-3 py-1 rounded-full text-xs font-bold uppercase text-text-main backdrop-blur">
                  New
                </span>
              )}
            ...
            {(!user || user.role === "buyer") && (
                <button
                  ...
                  className={wishlistButtonClass(isWishlisted)}
                >
                  {isWishlisted ? (
                    <HiHeart size={20} />
                  ) : (
